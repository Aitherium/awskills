...
```bash
# 0. Corpus, from per-directory READINGS of your repo: one JSON per line,
...
#    Output: one row per question -> right directory -> K hard negatives (plausible
#    and wrong), split by DIRECTORY so eval never sees a directory training saw.
#    Your own question -> target pairs go in via --ground-truth (JSONL {"query","target"}).
