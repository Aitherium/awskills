...
def _none_means_dead(mod: "ast.Module", func_name: str) -> bool:
    ...
    Deliberately narrow — it requires a real `is None` test whose branch ESCALATES
    to a DEAD verdict: `return 2`, `exit(2)`, `raise`, or a propagated
    `return None` (the collect-over-files idiom, measured 2026-08-28) — so a
    ...
    for node in ast.walk(mod):
        ...
        for inner in ast.walk(node):
            ...
            if isinstance(inner, ast.Return) and (
                ...
                # `return None` in the is-None branch: the function PROPAGATES
                # the DEAD verdict to its own caller — the honest idiom in
                # checkers whose entry point collects over many files and
                # exits 2 on the propagated None (measured 2026-08-28:
                # check_duplicate_top_level_decls.scan_paths).
                return True
            if isinstance(inner, ast.Raise):
                # `raise` escalates to DEAD — the idiom of
                # check_repo_root_doubling.run (raises DeadError; main exits 2).
                return True
...
#: HYG015 -- gates that MUST be wired in the debt-invariants workflow.
#: check_gate_coverage is the META-gate: CHK001-CHK007 assert the lanes
#: EXECUTE, so an unwired meta-gate makes the whole coverage plane
#: documentation again. Measured 2026-08-28 (the SECOND occurrence of the
#: family's self-eating shape): the "full coverage 570/570" work ported the
#: manifest but not the workflow half -- check_gate_coverage + run_gate_lane
#: appeared ZERO times in debt-invariants.yml for ~24h and CHK001-CHK007 ran
#: nowhere in CI, so every "coverage green" claim was unverifiable by
#: construction.
REQUIRED_WIRED = ("check_gate_coverage.py",)


def _hyg15(root: Path) -> list[str] | None:
    """HYG015 -- required-wired gates missing from the CI workflow.

    None, never []: an unreadable workflow means "could not judge" -- the
    same never-0-on-silence contract as HYG013.
    """
    wf = root / ".github/workflows/debt-invariants.yml"
    try:
        wired = wired_in_workflow(wf)
    except OSError:
        return None
    missing = [n for n in REQUIRED_WIRED if n not in wired]
    return [f"HYG015 {n}: required-wired gate missing from "
            f"debt-invariants.yml -- its enforcement plane runs nowhere in CI"
            for n in missing]
...
def run(root: Path, show_all: bool) -> int:
    ...
    for code, label, items in (
        ...
        ("HYG015", "required-wired gates missing from the CI workflow "
                   "(their enforcement plane runs nowhere)",
         (["HYG015 NOT VERIFIED: debt-invariants.yml could not be read"]
          if hyg15 is None else hyg15)),
    ):
        if items:
            findings = True
...
def _self_test_hyg10() -> list[str]:
    ...
    """
    if not LAYOUT.get("jest_app"):
        # A copy without the layout file (the public awskills twin) reports
        # NOT APPLICABLE by design — the arms below cannot build the fixture,
        # and a self-test that fails in a supported home is itself a defect
        # (measured 2026-08-28: the twin's home self-test was failing while
        # the canonical one passed, silently, because nobody ran it there).
        return []
    bad: list[str] = []
...
def _self_test_body() -> int:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        # HYG015: a workflow WITHOUT the required-wired meta-gate must be
        # flagged; WITH it, clean. Both directions, or the rule is noise.
        with tempfile.TemporaryDirectory() as _t15:
            _r15 = Path(_t15)
            (_r15 / ".github/workflows").mkdir(parents=True)
            _wf15 = _r15 / ".github/workflows/debt-invariants.yml"
            _wf15.write_text(
                "name: debt\non: {push: {branches: [develop]}}\n"
                "jobs:\n  j:\n    runs-on: ubuntu-latest\n    steps:\n"
                "      - run: python dev/tools/check_something.py\n",
                encoding="utf-8")
            _h15 = _hyg15(_r15)
            if not _h15 or not any("check_gate_coverage" in x for x in _h15):
                print("SELF-TEST FAIL: HYG015 did not flag a workflow without "
                      "the required-wired meta-gate", file=sys.stderr)
                return 1
            _wf15.write_text(
                "name: debt\non: {push: {branches: [develop]}}\n"
                "jobs:\n  j:\n    runs-on: ubuntu-latest\n    steps:\n"
                "      - run: python dev/tools/check_gate_coverage.py\n",
                encoding="utf-8")
            if _hyg15(_r15):
                print("SELF-TEST FAIL: HYG015 cried wolf on a workflow that "
                      "wires the meta-gate", file=sys.stderr)
                return 1
            _safe_print("  HYG015: ok - fires on a workflow missing the "
                        "required-wired meta-gate, silent when it is wired")
